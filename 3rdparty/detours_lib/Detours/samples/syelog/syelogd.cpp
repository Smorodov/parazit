...
VOID MyErrExit(PCSTR pszMsg)
{
	...
	LogMessageV(SYELOG_SEVERITY_FATAL, (PCHAR)"Error %d in %s.", error, pszMsg);
...
{
	LogMessageV(SYELOG_SEVERITY_INFORMATION,  (PCHAR)  (PCHAR)"Client closed pipe.");
...
PCLIENT CreatePipeConnection(HANDLE hCompletionPort)
{
	...
	{
		LogMessageV(SYELOG_SEVERITY_INFORMATION,  (PCHAR)
	...
	for (BOOL fKeepLooping = TRUE; fKeepLooping;)
	{
		...
		else if (!b)
		{
			if (pClient)
			{
				...
				{
					LogMessageV(SYELOG_SEVERITY_ERROR,  (PCHAR)
		...
		else
		{
			...
			{
				LogMessageV(SYELOG_SEVERITY_NOTICE,(PCHAR)
...
BOOL WINAPI ControlHandler(DWORD dwCtrlType)
{
	switch (dwCtrlType)
	{
		...
		case CTRL_SHUTDOWN_EVENT:
			LogMessageV(SYELOG_SEVERITY_INFORMATION,  (PCHAR)  (PCHAR)"User requested stop.");
